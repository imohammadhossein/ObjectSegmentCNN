...
def crop_circle(r, img, number, distance_thresh):
	...
	NOT = Image.composite(make_PIL(img), pil_img_back_2, test)

	return final_img, NOT
...
for m in range(5,143,3):
	if not os.path.exists(out_path + name + '/' + repr(m)):
	    os.makedirs(out_path + name + '/' + repr(m))
	overal_centers = []
	for i in range(100):
	    bubble_image, NOT = crop_circle(m, base_image, 13, distance_thresh)
	    # overal_centers.append(centers)
	    bubble_image.save(out_path+name+'/'+repr(m)+'/'+repr(i)+'.jpg')
	    NOT.save(out_path+name+'/'+repr(m)+'/'+repr(i)+'_NOT.jpg')
